...
def get_args():
    ...
    parser.add_argument('--source_data_path', type=str, default='./data/ATL-1K-121mo')
    parser.add_argument('--target_data_path', type=str, default='./data/ATL-3K-15mo')
...
def inference(args):
    ...
    source_dataset_name = source_data_path.split('/')[-1]
    target_dataset_name = target_data_path.split('/')[-1]
    folder_name = 'b'+str(batch_size)+'l'+str(num_layers)+'d'+str(hidden_dim)+'e'+str(num_epochs)+'_normal'+str(int(if_normalize))
    load_dir = os.path.join(args.save_path, source_dataset_name, args.encoder_type, folder_name)
    save_dir = os.path.join(args.save_path, target_dataset_name, args.encoder_type)
    ...
    model_name = 'model.pt'
    if not os.path.exists(os.path.join(load_dir, model_name)):
        raise Exception(f'{os.path.join(load_dir, model_name)} does not exist.')
    model.load_state_dict(torch.load(os.path.join(load_dir, model_name)))
    device = torch.device("cuda:0") if torch.cuda.is_available() else torch.device("cpu")
    model = model.to(device)

    train_dataset = TrajDailyDataset(data_path=target_data_path, train=True, max_daily_seq=max_daily_seq)
    test_dataset = TrajDailyDataset(data_path=target_data_path, train=False, max_daily_seq=max_daily_seq)
